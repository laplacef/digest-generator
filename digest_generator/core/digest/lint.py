...
watch" blocks, and mechanical residue the normalizer should have removed. Each
maps to a deterministic pattern, so a lint is a more reliable safety net than
re-reading by eye every publish cycle.
...
def lint_digest(markdown: str) -> list[LintFinding]:
